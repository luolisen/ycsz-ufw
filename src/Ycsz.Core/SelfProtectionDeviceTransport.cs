...
namespace Ycsz {
    ...
    public sealed class WindowsSelfProtectionTransport : ISelfProtectionTransport {
        ...
        [StructLayout(LayoutKind.Sequential, Pack=8)] struct NativeIdentity {
            public uint ProcessId; public uint Reserved; public long CreateTime100ns;
        ...
        [StructLayout(LayoutKind.Sequential, Pack=8)] struct NativeStatus {
            ...
            public uint TargetPid; public uint Reserved; public long TargetCreateTime100ns; public long MaintenanceExpiresAt100ns;
            [MarshalAs(UnmanagedType.ByValArray, SizeConst=16, ArraySubType=UnmanagedType.U1)] public byte[] LeaseId;
            [MarshalAs(UnmanagedType.ByValArray, SizeConst=32, ArraySubType=UnmanagedType.U1)] public byte[] ImageSha256;
            [MarshalAs(UnmanagedType.ByValArray, SizeConst=16, ArraySubType=UnmanagedType.U1)] public byte[] InstanceNonce;
            [MarshalAs(UnmanagedType.ByValArray, SizeConst=MaxPathChars, ArraySubType=UnmanagedType.U2)] public ushort[] ImagePath;
        ...
        static NativeActivateRequest BuildActivate(SelfProtectionRequest request) {
        ...
        static SelfProtectionReply QueryStatus(SafeFileHandle device,SelfProtectionRequest request) {
            ...
            try {
                ...
                return DecodeStatus(bytes,request,ToKernelPath(request.Identity.ImagePath),ToKernelPath(Path.GetDirectoryName(request.Identity.ImagePath)));
        ...
        public static SelfProtectionReply DecodeStatus(byte[] bytes,SelfProtectionRequest request,string expectedImage,string expectedRoot) {
            ...
            if(status.Size!=StatusSize || status.Version!=SelfProtectionProtocol.Version || status.Reserved!=0 ||
                (status.State&~(StateActive|StateProcessCallback|StateFileFilter|StateMaintenance|StateUnloadPrepared|StateError))!=0)
            ...
            if(status.TargetPid!=identity.ProcessId || status.TargetCreateTime100ns!=identity.StartTimeUtcFileTime ||
            ...
            } else if(maintenance || (status.State&StateUnloadPrepared)!=0 || status.MaintenanceExpiresAt100ns!=0 || !EqualBytes(status.LeaseId,new byte[16])) {
                throw new InvalidDataException("驱动尚未退出维护状态");
            }
            SelfProtectionCapability capabilities=SelfProtectionCapability.None;
            if(!maintenance) {
                if((status.State&StateProcessCallback)!=0) capabilities|=SelfProtectionCapability.ProcessTermination;
