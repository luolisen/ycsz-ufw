...
- 卸载由 Filter Manager 回调管理，不再动态修改 `DriverUnload`。注册标志拒绝 SCM 服务停止卸载；非强制卸载在实际回调时核验有效维护租约与 PREPARE_UNLOAD。强制卸载回调不能否决，必须清理。控制设备按 FILE_OBJECT 计数，普通卸载拒绝仍有引用的连接，CLEANUP 不提前释放引用；强制卸载与实际 Windows I/O 完成并发仍未验收，不能据此安装到生产。应用已接入用户态 `PREPARE_UNLOAD` 请求路径，但真实驱动卸载流程仍未动态验收。
