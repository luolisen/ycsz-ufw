...
| 验证环境 | 结果 |
| --- | --- |
| Windows CI（33fe669） | 76 项 C# 回归、2 项真实回环 TLS、9 项隔离网络逻辑测试通过；PowerShell 语法检查和四种 NSIS 打包通过；9 种运行库处理分支测试通过 |
| 一次性 Windows 安全测试环境 | 43 项检查通过：标准用户权限限制、管理认证、服务进程终止句柄、服务停止权限、安装文件删除/覆盖等；另验证 SCM 恢复与设备状态持久化 |
| STU01 学生端（2026-09-13） | 已更新33fe669并核对哈希；原密码登录、托盘结束后恢复且核心PID不变、认证维护开关通过。当前登录账户为管理员，不能据此声称标准用户防护已在该账户生效。见[本次实机记录](docs/validation/DRIVERLESS-REMOTE-2026-09-13.md) |
| 教师端（2026-09-13） | 已更新33fe669，程序及两种客户端安装器哈希匹配；桌面快捷方式、原密码登录、STU01在线和运行库选择界面通过。本次实际双包导出由CI验证，生产未新增接入包 |
